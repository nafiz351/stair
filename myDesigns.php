...
::-webkit-scrollbar-thumb:hover {
 /* background: #df4759; */
  ...
  <?php include "includes/Filternavigation.php"; ?>
	...
	//{   ?>
                  <div class="dropdown-divider"></div>
                  <a class="dropdown-item preview-item" id="mdes"  href="customerDesigns.php">
                    <div class="preview-thumbnail">
                      <div class="preview-icon bg-dark rounded-circle">
        ...
        <div class="main-panel">
		  ...
		  <form method="post"  >
            <div class="row" id="filterdata"  >
			  	<?php
$ret=mysqli_query($con,"select * from user_designs WHERE userid='".$_SESSION['id']."'");
$countcol=0;
                        ...
                        <div class="d-flex align-items-left align-self-start">
                          ...
                          <div class="btn-group" role="group" aria-label="Basic example">
                            <button type="button" onclick="like(this.id);" id="<?php echo $row['id'];?>" class="btn btn-outline-secondary">
									    	<?php
							$thisid=$row['id'];
$retl=mysqli_query($con,"select * from likes WHERE design_id='$thisid' AND user_id='".$_SESSION['id']."' AND design_type='user'");
 ...
 <?php
	...
	?>
                      ...
                      <div class="col-2">
                        <div class="d-flex align-items-center align-self-start">
                              <i class="mdi mdi-heart"></i>
					<?php	$retc=mysqli_query($con,"select * from likes WHERE design_id='$thisid' AND design_type='user' AND `user_id`='".$_SESSION['id']."'");
			$likecount=0;
...
function edit(va){
	var id=va;
	//alert(id);
  window.location.replace('index.php?user_design='+id);
}
function like(did){
	var mid=did;
    var uid="<?php echo $_SESSION['id'];?>";
